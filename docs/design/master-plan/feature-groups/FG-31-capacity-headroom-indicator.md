...
**Wave:** P6-E (independent; can run any time after FG-28) · **Owner agent:** devin · **Status:** DONE — implemented (#260), calibrated and system-tested on `hermes-systest` 2026-08-16 at `a56bc4a6f` (#264): `conversation_cost_mb` 250 → 20, measured (§Calibration status). One decision is open and is Leo's: `max_concurrent_sessions` stays **per profile**, so the box can be asked for 3 × 15 — the indicator reports the box-wide total and enforces nothing
...
|------|---------|--------|--------|-----------|
| 2026-08-16 | 4 | devin (for Leo) | Phase-6 close-out: status corrected from "awaiting the calibration load run" to DONE, with the open cap decision moved into the header | The run happened on 2026-08-16 and its result is in this file's own checklist; only the header had not caught up. The one genuinely open item is not work but a decision — `max_concurrent_sessions` is per profile, so three profiles can be asked for 45 concurrent conversations while the indicator reports the box-wide total and enforces nothing — so it belongs where it is read, not in a section. |
