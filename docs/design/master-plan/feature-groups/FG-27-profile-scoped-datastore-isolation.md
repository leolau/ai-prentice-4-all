...
**Wave:** P6-0 (Phase-6 **prerequisite** — lands before FG-25/FG-26 touch app tables) · **Owner agent:** _unassigned_ · **Status:** DONE — implemented, deployed and system-tested on `hermes-systest` 2026-08-12 at `76fb221eb` (§System testing); two sub-items deferred and named in the checklist (`schema` override, `datastore claim --force` CLI)
...
- [x] Layer 2 (**last, UX**) — `--clone` / `--clone-all` / `import` / `install` resolve the new profile's `(database, schema)` from the source's *effective* DSN, print it, and refuse an already-claimed schema before anything is written (`hermes_cli/datastore_binding.py`, `check_datastore_binding()`)
...
1. **An unreachable database is a note, not a refusal.** The spec's Layer 2
   verifies the claim before creating a profile; if Postgres is down there is no
   claim to read. Failing closed there would make a database outage take the
   whole profile-management surface with it, while Layer 1 already refuses the
   first connection — so creation continues and the report says ownership was
   not verified.
2. **`split-profile` moves whole schemas only, and never a subset.** The command
   the spec asks for ("refuse with a clear explanation if rows are already
   interleaved") is implemented as three refusals: a source schema claimed by a
   third profile, a populated target schema, and a post-move row-count
   mismatch (rolled back inside the transaction). There is deliberately no
   flag that extracts one profile's rows out of a shared schema — those rows
   carry no provenance column, so any such split would be a guess.
3. **`hermes datastore show` was added** alongside `split-profile`. The doctor
   check reports the same facts, but an operator asking "which database am I
   on?" should not have to run a whole health sweep to find out — and the DSN is
   indirect, so config cannot answer it.
4. **`hermes profile install` is checked too.** Not named in the spec, but a
   distribution brings its own `config.yaml`, which is exactly the inheritance
   `--clone` has.
...
|------|---------|--------|--------|-----------|
| 2026-08-16 | 3 | devin (for Leo) | Phase-6 close-out: status corrected from `PLAN — not started` to DONE, with the two deferred sub-items named in the header | Every checklist item was ticked and the system test recorded, but the header a cold agent reads first said the FG had not begun. No content change. |
